...
class TextPredictionModel:
    ...
    def from_artefacts(cls, artefacts_path: str):
        ...
        # TODO: CODE HERE
        # load params
        path_params = os.path.join(artefacts_path, 'params.json')
        params = json.load(open(path_params))
        # TODO: CODE HERE
        # load labels_to_index
        path_labels = os.path.join(artefacts_path, 'labels_index.json')
        labels_to_index = json.load(open(path_labels))
    ...
    def predict(self, text_list, top_k=5):
        ...
        # TODO: CODE HERE
        # embed text_list
        embed_text = embed(text_list)
        # TODO: CODE HERE
        # predict tags indexes from embeddings
        tag_pred = self.model.predict(embed_text)
        # TODO: CODE HERE
        # from tags indexes compute top_k tags for each text
        predictions = [self.labels_index_inv[label] for label in argsort((-1)*tag_pred)[0][:top_k]]
