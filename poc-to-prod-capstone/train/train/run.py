...
def train(dataset_path, train_conf, model_path, add_timestamp):
    ...
    with open(os.path.join(artefacts_path, 'params.json'), 'w') as fp:
        json.dump(train_conf, fp)
    # TODO: CODE HERE
    # save labels index in artefacts_path/labels_index.json
    with open(os.path.join(artefacts_path,'labels_index.json'), 'w') as fp:
        json.dump(dataset.get_label_to_index_map(), fp)
