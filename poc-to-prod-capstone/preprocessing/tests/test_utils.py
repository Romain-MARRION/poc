...
class TestBaseTextCategorizationDataset(unittest.TestCase):
    ...
    def test__get_num_test_batches(self):
        ...
        # we mock _get_num_samples to return the value 100
        base._get_num_samples = MagicMock(return_value=100)
        # we assert that _get_num_train_batches will return  _get_num_test_samples / batche = 1
        self.assertEqual(base._get_num_test_batches(), 1)
        
    def test_get_index_to_label_map(self):
        # TODO: CODE HERE
        base = utils.BaseTextCategorizationDataset(20, 0.8)
        base._get_label_list = MagicMock(return_value=['chien','chat','citron'])
        expected={0:'chien',
                  1:'chat',
                  2:'citron'
                  }
    ...
    def test_index_to_label_and_label_to_index_are_identity(self):
        ...
        base = utils.BaseTextCategorizationDataset(20, 0.8)
        base._get_label_list = MagicMock(return_value=['chien','chat','citron'])
        dic=base.get_label_to_index_map()
        for v,k in base.get_index_to_label_map().items():
            self.assertEqual(dic[k],v)
        
    def test_to_indexes(self):
        # TODO: CODE HERE
        base = utils.BaseTextCategorizationDataset(20, 0.8)
        base.get_label_to_index_map=MagicMock(return_value={'chien':0,'chat':1,'citron':2})
        expected=[0,1,1,2,0]
        self.assertEqual(base.to_indexes(['chien','chat','chat','citron','chien']), expected)
...
class TestLocalTextCategorizationDataset(unittest.TestCase):
    ...
    def test__get_num_samples_is_correct(self):
        ...
        pd.read_csv = MagicMock(return_value=pd.DataFrame({
            ...
            'tag_name': ['tag_a', 'tag_b','tag_b', 'tag_a', 'tag_a'],
            'tag_id': [1, 2, 2, 1, 1],
            'tag_position': [0, 0, 1, 0, 0],
            'title': ['title_1', 'title_2','title_3', 'title_4', 'title_5']
        }))
        dataset = utils.LocalTextCategorizationDataset("fake_path", 1, min_samples_per_label=1,train_ratio=0.7)
    ...
    def test_get_train_batch_returns_expected_shape(self):
        ...
        pd.read_csv = MagicMock(return_value=pd.DataFrame({
            ...
            'tag_name': ['tag_a', 'tag_b','tag_b', 'tag_a', 'tag_a'],
            'tag_id': [1, 2, 2, 1, 1],
            'tag_position': [0, 1, 2, 3, 5],
            'title': ['title_1', 'title_2','title_3', 'title_4', 'title_5']
        }))
        dataset = utils.LocalTextCategorizationDataset("fake_path", batch_size=1, min_samples_per_label=0,train_ratio=0.7)
    ...
    def test_get_train_batch_raises_assertion_error(self):
        ...
        with self.assertRaises(AssertionError):
            utils.LocalTextCategorizationDataset("fake_path", batch_size=1, min_samples_per_label=5,train_ratio=0.7)
